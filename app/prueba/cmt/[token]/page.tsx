...
import { ChevronLeft, ChevronRight, Clock, AlertCircle, Check, ChevronDown, ChevronUp } from "lucide-react"
...
// Componente de Instructivo (similar a OnboardingScreen)
function InstructivoCMT({ onStart }: { onStart: () => void }) {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white relative overflow-x-hidden">
      {/* Decoración de fondo con astronauta */}
      <motion.div 
      ...
      <section className="min-h-screen flex flex-col justify-center items-center px-4 py-16 md:py-20 relative">
        <motion.div 
          className="absolute bottom-20 left-8 hidden lg:block opacity-10"
          animate={{ 
            scale: [1, 1.05, 1],
            x: [0, 5, 0]
          }}
        ...
        >
          ...
          <p className="text-xl md:text-2xl text-primary font-semibold mb-8 text-center">
            CMT
          </p>
          <div className="space-y-5 text-gray-700 leading-relaxed">
            <p className="text-base md:text-lg text-center">
              Este cuestionario <strong>no es un examen</strong>. No hay respuestas correctas o incorrectas.
              Queremos conocer qué te motiva en el trabajo para ubicarte mejor en el cargo.
            </p>
            <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl">
              <p className="text-sm md:text-base text-blue-900 text-center">
                ...
                proceso de selección y son completamente privadas.
              </p>
            </div>
          </div>
      ...
      <section className="min-h-screen flex items-center py-16 md:py-20 px-4 relative">
        <motion.div 
          className="absolute top-32 right-8 hidden lg:block opacity-10"
          animate={{ 
            y: [0, -15, 0]
          }}
          transition={{ 
          ...
          >
            ...
            <div className="space-y-4 text-gray-700 text-sm md:text-base leading-relaxed">
              ...
              <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-4">
                <p className="font-semibold text-blue-900 mb-3">📋 Tu tarea es ordenar las 5 frases según tu preferencia:</p>
                <ul className="space-y-2 ml-2">
                  <li className="flex items-start gap-2">
                    <span className="flex-shrink-0 w-8 h-8 rounded-full bg-blue-600 text-white font-bold flex items-center justify-center">5</span>
                    <span>La frase que te produce <strong>MAYOR satisfacción</strong> o que MÁS te representa en el trabajo.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    ...
                    <span>La que te satisface <strong>bastante</strong>.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="flex-shrink-0 w-8 h-8 rounded-full bg-blue-400 text-white font-bold flex items-center justify-center">3</span>
                    <span>La que te satisface <strong>moderadamente</strong>.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="flex-shrink-0 w-8 h-8 rounded-full bg-blue-300 text-white font-bold flex items-center justify-center">2</span>
                    <span>La que te satisface <strong>poco</strong>.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="flex-shrink-0 w-8 h-8 rounded-full bg-blue-200 text-gray-700 font-bold flex items-center justify-center">1</span>
                    <span>La frase que te produce <strong>MENOR satisfacción</strong> o que MENOS te representa.</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-amber-50 border-l-4 border-amber-400 p-4 rounded-r-lg">
                ...
                <ul className="list-disc list-inside space-y-1.5 ml-2 text-amber-800">
                  <li><strong>Debes usar todos los números del 5 al 1</strong> en cada grupo (uno por cada frase).</li>
                  <li><strong>No puedes repetir números</strong> dentro del mismo grupo.</li>
              ...
              <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded-r-lg">
                ...
                <ul className="list-disc list-inside space-y-1.5 ml-2 text-green-800">
                  ...
                  <li>Tiempo estimado: <strong>20-35 minutos</strong> </li>
      ...
      <section className="min-h-screen flex items-center py-16 md:py-20 px-4 relative">
        <motion.div 
          className="absolute bottom-32 right-12 hidden lg:block opacity-10"
          animate={{ 
            rotate: [0, 8, -8, 0],
            y: [0, -12, 0]
          }}
        ...
        >
          ...
          <Button
            ...
            onClick={onStart}
            size="lg"
            className="w-full sm:w-auto px-10 py-6 text-lg font-semibold shadow-lg hover:shadow-xl transition-shadow"
          >
            Comenzar Prueba CMT
          </Button>
...
export default function PruebaCMTPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    // Compatibilidad con API anterior: usar fechaLimite si fechaLimiteUtc no viene
    ...
    // Si no hay deadline, no iniciar reloj
  ...
  const handleSeleccionarValor = (letra: string, valor: number) => {
    if (sesionExpirada) {
      toast({ title: 'Tiempo excedido', description: 'La prueba ha expirado. No puedes cambiar el ranking.', variant: 'destructive' })
      return
    }
    ...
    if (validarRanking(nuevoRanking)) {
      ...
      // Si ya no está completo, remover de respondidas para actualizar índice
      setRespuestas((prev) => {
        const nueva = new Map(prev)
  ...
  const validarRanking = (ranking: Record<string, number> | undefined): boolean => {
    ...
    // Debe haber exactamente 5 opciones
    if (letras.length !== 5) return false
    
    // Todos los valores del 1 al 5 deben estar presentes
    const valoresUnicos = new Set(valores)
    if (valoresUnicos.size !== 5) return false
    
    // Verificar que sean los valores correctos (1-5)
    for (let i = 1; i <= 5; i++) {
  ...
  const handleGuardarRanking = () => {
    // Guardar respuesta actual si el ranking está completo
  ...
  const limpiarRankingActual = () => {
    setRankingTemporal({})
    // remover respuesta guardada si existía
    const numeroEnunciado = preguntaActual + 1
    setRespuestas((prev) => {
      const nueva = new Map(prev)
      nueva.delete(numeroEnunciado)
  ...
  const cambiarPregunta = (nuevaPregunta: number) => {
    ...
    // Cargar ranking de la nueva pregunta si existe
  ...
  const handleEnviarRespuestas = async () => {
    if (sesionExpirada) {
      toast({ title: 'Tiempo excedido', description: 'No es posible enviar respuestas porque la sesión expiró.', variant: 'destructive' })
      return
    }
    // Construir mapa final con todas las respuestas guardadas + la actual si está completa
    let respuestasFinales = new Map(respuestas)
    
    // Si la respuesta actual está completa y no ha sido guardada, agregarla
    if (validarRanking(rankingTemporal)) {
      const numeroEnunciado = preguntaActual + 1
    ...
    // Validar que tengamos las 15 respuestas
    ...
    // Validar que cada respuesta tenga un ranking válido
    ...
    const respuestasArray = Array.from(respuestasFinales.values()).sort(
      (a, b) => a.numeroEnunciado - b.numeroEnunciado
    )
    ...
    try {
      ...
      if (result.success) {
        ...
        // Redirigir a página de éxito SIN recargar preguntas
        const psicologoEmail = result.data?.psicologoEmail
        if (psicologoEmail) {
          localStorage.setItem(`cmt_psicologo_email_${token}`, psicologoEmail)
          // Usar window.location para navegación completa y evitar re-fetch de preguntas
          window.location.href = `/prueba/cmt/${token}/exito?psicologoEmail=${encodeURIComponent(psicologoEmail)}`
        } else {
          window.location.href = `/prueba/cmt/${token}/exito`
        }
      }
    } catch (error) {
          ...
          <div className="bg-white rounded-lg shadow-md p-3">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs text-gray-600">
                <span>Progreso: {respuestas.size} de 15 preguntas</span>
              ...
              <CardHeader className="bg-gradient-to-r from-blue-50 to-blue-100 py-2 px-3">
                <div className="flex justify-between items-start mb-1.5">
                  <span className="text-xs font-semibold text-blue-700">
                    Pregunta {pregunta.numeroEnunciado} de 15
                  </span>
