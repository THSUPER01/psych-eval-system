...
export default function AdminUsuariosPage() {
  ...
  const [formData, setFormData] = useState<RegistrarUsuarioForm>({
    documentoUsuario: "",
    idRol: 0,
  })
  const [errors, setErrors] = useState<Partial<RegistrarUsuarioForm>>({})
  ...
  const registrarMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpenDialog(false)
      setFormData({ documentoUsuario: "", idRol: 0 })
      setErrors({})
    },
    onError: (error: any) => {
      toast({
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<RegistrarUsuarioForm> = {}
    
    if (!formData.documentoUsuario || formData.documentoUsuario.length < 5) {
      newErrors.documentoUsuario = "Documento debe tener al menos 5 caracteres"
    }
    
    if (!formData.idRol || formData.idRol === 0) {
      newErrors.idRol = 1 // Indica que hay error
    }
  ...
  const updateFormData = (field: keyof RegistrarUsuarioForm, value: string | number) => {
            ...
            <form onSubmit={handleSubmit} className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={formData.documentoUsuario}
                  onChange={(e) => updateFormData("documentoUsuario", e.target.value)}
                />
                {errors.documentoUsuario && (
                  <p className="text-sm text-destructive">{errors.documentoUsuario}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => updateFormData("idRol", Number(value))}
                >
                  <SelectTrigger>
                ...
                {errors.idRol && (
                  <p className="text-sm text-destructive">Debe seleccionar un rol</p>
                )}
              </div>
